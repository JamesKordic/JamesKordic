...
/** Headline split into words for the staggered mask reveal — the four
 *  industries pop in the accent color, and the closing "Selected work below."
...
const HEADLINE_WORDS = [
  ...
  ...'Selected work below.'.split(' ').map((t) => ({ t, cls: 'text-muted' })),
];

/**
 * Home — a Pentagram-style editorial index: a single oversized intro line
 * beneath the sticky name header, then a bordered grid of project tiles.
          ...
          {HEADLINE_WORDS.map((w, i) => (
            <span key={i}>
              <span className="word-reveal">
                <span className={w.cls} style={{ animationDelay: `${i * 32}ms` }}>
                  {w.t}
                </span>
              </span>{' '}
            </span>
